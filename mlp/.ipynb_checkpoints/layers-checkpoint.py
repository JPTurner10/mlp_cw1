    ...
    def fprop(self, inputs, stochastic=True):
        ...
        if stochastic:
            if self.share_across_batch:
                mask = self.rng.uniform(0,1, size=inputs.shape[1:])
                mask = np.repeat(np.expand_dims(mask, axis=0), inputs.shape[0], axis=0)
                mask = mask < self.incl_prob
                self.mask = mask
                return np.multiply(mask, inputs)
            else:
                mask = self.rng.uniform(0,1, size=inputs.shape)
                mask = mask < self.incl_prob
                self.mask = mask
                return np.multiply(mask, inputs) / (1-self.incl_prob)
        else:
